...
请输出 JSON 格式（只输出 JSON）：
{
  ...
  /**
   * Step 5: 创意方向 - 精简版（只传递核心字段）
   */
  creativeDirectionsCompact: (bookDNA, audienceProfile, sellingPoints) => {
    // 只提取核心字段
    const coreInfo = {
      genre: bookDNA.bookAnalysis?.basicInfo?.genre,
      hook: bookDNA.bookAnalysis?.basicInfo?.coreHook,
      lead: bookDNA.bookAnalysis?.characters?.protagonist?.name,
      age: audienceProfile.primary?.age,
      pain: audienceProfile.primary?.painPoints?.slice(0, 3)
    };
    
    const topPoints = sellingPoints.finalSellingPoints?.slice(0, 3).map(sp => ({
      id: sp.pointId,
      title: sp.title,
      pitch: sp.oneLinePitch
    }));
    
    return `你是网文广告创意专家。用最直白、最爽的网文语言写创意。

核心信息:
- 类型: ${coreInfo.genre}
- 钩子: ${coreInfo.hook}
- 主角: ${coreInfo.lead}
- 受众: ${coreInfo.age}, 痛点: ${coreInfo.pain?.join(', ')}

卖点:
${topPoints?.map((p, i) => `${i+1}. ${p.title}: ${p.pitch}`).join('\n')}
...
输出格式:
{"creativeDirections":[{"pointId":"SP001","creativeName":"创意名称","creativeHook":"第一秒钩子","tiktok":{"videoScript":"脚本","caption":"配文","hashtags":["#标签"]},"facebook":{"headline":"标题","primaryText":"文案"},"visualBrief":"视觉描述","emotionalAngle":"情感切入点"}]}`;
  }
};
